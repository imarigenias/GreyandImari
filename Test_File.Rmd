...
```{r label-variable-names,}
    ...
    # LEDD is saved as "Shantha-made column" for some reason
```
...
Re-factor certain variables. Doesn't work with creating variable labels. 
```{r select-binary-variables,}
...
=======
  summary(tableby(~ genderf + age_enrollment , data=sdLabels), title="Demographics")
  summary(tableby(~ as.factor(gender) + age_enrollment , data=sdLabels), title="Demographics")
  
  summary(tableby(~ gender + age_enrollment , data=sdFactors), title="Demographics")
...
```{r}
    summary(tableby(~gender + age_enrollment , data=sdFactors), title="Demographics")
>>>>>>> 0c4ef1bbf07f834aa016bdf05feb88464e602d79
```

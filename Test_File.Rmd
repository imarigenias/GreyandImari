...
## Create environ file
On R Studio: File > New File > New Text File. Save the file as ".Renviron." Make sure is is saved in the project directory. 
...
redcap_data = ReadRedcapReport(token = Sys.getenv("clinic_redcap_token"), url = "https://redcap.emory.edu/api/", report_id = 32116)

redcap_data

```
`redcap_data=ReadRedcapReport`creates a data frame called `redcap_data` using the `ReadReadcapReport` function from the `motionTools` package. I created it without manipulating the data set first, so I can show you what information it displays and how. 
...
```{r}
  print("Select specific column --> df_name$col_name")
  patients$ID
  
  print('Select specific column --> df[num_row, ]')
  patients[1, ]
...
### Counts
```{r}
  print("Num of patients with dosage above 100 (TRUE)")
  table(patients$Dosage > 100)
  
  print("Number of missing values (TRUE)")
...
```{r}
  ...
  print("Summary of certain variables")
...
```{r}
  print("Display IDs and ages of subjects with missing Ages")
  ids_ageNA<- patients |>
    filter(is.na(Age))|>  
    select(ID, Age)
...
Record ID 
```{r}
  # record IDs 3 and 5 have missing Ages
  # We want ID #3 to have an age of 50
  patients[patients$ID =="03", "Age"] <- 50 
  patients[3,]

  # Also, want patient 5 to be 70
  patients$Age[5] <- 70
  patients[5,]
...
Add a line of text 
I don't understand what I am doing but here goes.
